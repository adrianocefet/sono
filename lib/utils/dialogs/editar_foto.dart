...
class EditarFoto extends StatefulWidget {
  ...
  const EditarFoto(this.idIdentificador, this.identificador, {Key? key})
      : super(key: key);
...
class _EditarFotoState extends State<EditarFoto> {
                        ...
                        builder: (context) => Column(
                              ...
                              children: [
                                ListTile(
                                  ...
                                  title: Text("Câmera"),
                                  onTap: () async {
                                    XFile? _imagem = await FirebaseService()
                                        .selecionarArquivoCamera();
                                    if (_imagem != null &&
                                        _imagem.path.isNotEmpty) {
  ...
  void confirmarFoto(
      XFile imagem, String idIdentificador, String identificador) {
    String? singleImage;
    showDialog(
      context: context,
              ...
              TextButton(
                  ...
                  onPressed: () async {
                    ...
                    if (identificador == "Equipamento") {
                      singleImage = await FirebaseService()
                          .uparArquivoEquipamento(imagem, idIdentificador);
                      await FirebaseService().atualizarFotoEquipamento(
                          idIdentificador, singleImage!);
                    } else {
                      singleImage = await FirebaseService()
                          .uparArquivoPaciente(imagem, idIdentificador);
                      await FirebaseService()
                          .atualizarFotoPaciente(idIdentificador, singleImage!);
                    }
