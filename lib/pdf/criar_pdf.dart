...
class PdfInvoiceApi{
  ...
  static Future<File> gerarTermoDeResponsabilidade(Solicitacao solicitacao, Paciente pacienteEmprestado, Equipamento equipamentoEmprestado, UserModel usuario)async{
    final pdf = Document();
    pdf.addPage(MultiPage(
      build: (context) => [
        titulo(TipoSolicitacao.emprestimo),
        solicitante(pacienteEmprestado, equipamentoEmprestado, TipoSolicitacao.emprestimo),
  ...
  static Future<File> gerarTermoDeDevolucao(Solicitacao solicitacao,Paciente pacienteEmprestado, Equipamento equipamentoEmprestado, UserModel usuario)async{
    final pdf = Document();

    pdf.addPage(MultiPage(
      build: (context) => [
        titulo(TipoSolicitacao.devolucao),
        solicitante(pacienteEmprestado, equipamentoEmprestado, TipoSolicitacao.devolucao),
        equipamento(equipamentoEmprestado,solicitacao, usuario),
        paciente(pacienteEmprestado),
      ]
      ));
...
Widget equipamento(Equipamento equipamentoEmprestado, Solicitacao solicitacao,UserModel dispensacao) {
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      ...
      SizedBox(height: 0.8*PdfPageFormat.cm),
      Text('Fortaleza, CE, ${PdfInvoiceApi().dataAtualFormatada}',
        textAlign: TextAlign.left,
        style: TextStyle(
          fontSize: 11, fontWeight: FontWeight.normal
        )
      ...
      Text('Razão para devolução dos itens: ${solicitacao.justificativaDevolucao}',
        textAlign: TextAlign.left,
        style: TextStyle(
          fontSize: 11, fontWeight: FontWeight.normal
        )
...
Widget solicitante(Paciente pacienteEmprestado, Equipamento equipamento,TipoSolicitacao tipoSolicitacao) {
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
...
Widget titulo(TipoSolicitacao tipoSolicitacao) {
  return Column(
    ...
    children: [
      ...
      SizedBox(height: 0.5*PdfPageFormat.cm),
      Text(tipoSolicitacao==TipoSolicitacao.emprestimo?'DOCUMENTO DE COMPROVAÇÃO DE ENTREGA':'DOCUMENTO DE DEVOLUÇÃO DE ITENS',
        textAlign: TextAlign.center,
        style: TextStyle(
          fontSize: 11, fontWeight: FontWeight.normal,
        )
